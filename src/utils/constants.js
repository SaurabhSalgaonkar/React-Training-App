const MY_HOTEL_NAME = "Saurabh's Hotel Chain";
const HOTEL_LOGO = "https://imgak.mmtcdn.com/pwa_v3/pwa_hotel_assets/header/mmtLogoWhite.png"

const NAV_HOME = "Home";
const NAV_HOTELS = "Hotels";
const NAV_CONTACT_US = "Contact Us";

const TWITTER_IMG = "data:image/png;base64,";
const FB_IMG = "data:image/png;base64,"

export {
    MY_HOTEL_NAME,
    NAV_HOME,
    NAV_HOTELS,
    NAV_CONTACT_US,
    TWITTER_IMG,
    FB_IMG,
    HOTEL_LOGO
}